...
func facebookAssociate(w http.ResponseWriter, r *http.Request) {
	...
	case err != nil && err == lib.BadFBToken:
		go api.Statsd.Count(1, "gleepost.profile.facebook.post.400")
		jsonResponse(w, err, 400)
	case err != nil:
		go api.Statsd.Count(1, "gleepost.profile.facebook.post.500")
...
func facebookHandler(w http.ResponseWriter, r *http.Request) {
	...
	case err == lib.FBNoEmail:
		go api.Statsd.Count(1, "gleepost.facebook.post.400")
		jsonResponse(w, err, 400)
		return
	case err != nil:
		go api.Statsd.Count(1, "gleepost.facebook.post.500")
		jsonErr(w, err, 500)
		return
	case status.Status == "registered":
		...
		go api.Statsd.Count(1, "gleepost.facebook.post.200")
		jsonResponse(w, status, 200)
	case status.Status == "unverified":
		go api.Statsd.Count(1, "gleepost.facebook.post.201")
