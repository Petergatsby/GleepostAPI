...
func main() {
	...
	convs := base.PathPrefix("/conversations").Subrouter()
	convs.HandleFunc("/live", liveConversationHandler)
	convs.HandleFunc("/read_all", readAll).Methods("POST")
	convs.HandleFunc("/read_all/", readAll).Methods("POST")
	convs.HandleFunc("/{id:[0-9]+}", getSpecificConversation).Methods("GET")
	convs.HandleFunc("/{id:[0-9]+}/", getSpecificConversation).Methods("GET")
	convs.HandleFunc("/{id:[0-9]+}", putSpecificConversation).Methods("PUT")
	convs.HandleFunc("/{id:[0-9]+}", deleteSpecificConversation).Methods("DELETE")
	convs.HandleFunc("/{id:[0-9]+}/messages", getMessages).Methods("GET")
	convs.HandleFunc("/{id:[0-9]+}/messages", postMessages).Methods("POST")
	convs.HandleFunc("/{id:[0-9]+}/messages", putMessages).Methods("PUT")
